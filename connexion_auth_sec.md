...
|Protection microsoft | Description |references| 
| ------|-----|-----|
|Politique des mots de passe|La politique de mots de passe est très importante en particulier sur les comptes à privilèges. En cas de vol de la base SAM, plus le mot de passe est complexe, plus l'attaquant va mettre du temps à l'obtenir. Cette politique doit donc forcement aussi prendre en compte un delai de renouvellement adapté à la complexiter des mots de passe: (![image](https://user-images.githubusercontent.com/1132448/214883434-4548e6e3-6333-4d87-a2aa-3cde7f66f88c.png)|https://learn.microsoft.com/fr-fr/windows/security/threat-protection/security-policy-settings/password-policy|
|LAPS|Chaque machine du SI obtient un mot de passe d'administrateur local unique et fort. Cela evite la latérlisation avec des comptes locaux.|https://learn.microsoft.com/fr-fr/windows-server/identity/laps/laps-overview|
|Groupe AD "Protected users"|Chaque utilisateur dans ce groupe sera protégé contre l'utilisation de protocoles faibles. Voici les details:<ul><li>Expiration TGT après 4h au lieu de 10 heures</li><li>Authentification forcée par kerberos (interdiction NTLM)</li><li>Interdiction pour les machines de mettre en cache l'identifiant (cela veut dire qu'il n'est pas possible d'utiliser le compte si la machine n'accède pas aux controleurs de domaine)</li><li>Auth Wdigest désactivée</li><li>utilisation d'algorithmes de chiffrement faibles désactivée (RC4 & DES)</li><li>Le compte ne peut pas être délégué (équivalent à l'activation de "Account is sensitive and cannot be delegated") ou avoir l'attribut SPN</li></ul> |https://learn.microsoft.com/fr-fr/windows-server/security/credentials-protection-and-management/protected-users-security-group|
|Blindage kerberos|La creation d'un TGT sera sécurisée par l'utilisation du secret de la machine. Cela évite le risque de creation de TGT par les faiblesse sur les échanges AS-REP|https://learn.microsoft.com/fr-fr/windows-server/identity/ad-fs/operations/ad-fs-compound-authentication-and-ad-ds-claims|
|Authentification composée|L'authentification composée permet de créer des strategies d'authentification afin qu'un compte spécifique ne soit utilisable que depuis une machine précise (le poste d'administration). Toutes tentatives d'authentification "interactive" sur une autre machine que cette dernière echouera. Cela permet de garantir que même en cas de perte d'un identifiant seule la machine légitime puisse l'utiliser et personne d'autre.|https://learn.microsoft.com/fr-fr/windows-server/identity/ad-fs/operations/ad-fs-compound-authentication-and-ad-ds-claims|
...
**Bonnes pratiques d'administration windows pour éviter le vol de ses secrets :**
 ...
 - Réaliser des délégations de droits sur les "OU" spécifiques et n'utiliser pas les comptes d'administrateur du domaine pour administrer tout le parc
 - Un compte d'administration (compte à privilèges du domaine) ne doit être utilisé que sur le poste de l'administrateur (poste durci en matière de sécurité : politique d'exécution, politique firewall local, chiffrement des disques, pas d'accès bureautique pour limiter la surface d'attaque, ...). La compromission d'un poste d'administration peu importe le type d'authentification utilisé (carte à puce, fido, ...) et les protections mises en place sur le SI (bastion, politique d'authentification, segmentation, ...) rend "compromettable" toutes les ressources administrées par ce poste (niveau, ex: tiers 1). Dans le même principe, un contrôleur de domaine compromis rend "compromettable" toutes les ressources utilisant ce moyen d'authentification (tous les niveaux tiers0/1/2 - sauvegarde, hyperviseur, machine, service, ...).
 - L'administration des serveurs et postes distant ne doit être réalisée que depuis ces postes d'administration. Lors de l'administration des serveurs et postes à distance, il est obligatoire de n'utiliser que les 4 possibilités suivantes : wmi distant, powershell distant, psexec sans l'option "-u", rdp avec l'option obligatoire "/RestrictedAdmin" ou rdp avec le compte d'administration local de machine distante. L'option "/RestrictedAdmin" du rdp permet de se connecter à distance sans risque de vol d'identifiant car il ouvrira une session en tant qu'administrateur local et non pas avec les droits d'administrateur du réseau. Il est interdit par la suite de faire "executer en tant que" pour retrouver des droits d'administrateur du réseau/domaine sur la machine distante (afin de ne pas risquer le vol d'identifiants). Il est possible d'administrer un poste ou un serveur en local (physiquement) en utilisant le compte d'administrateur local du serveur ou poste, mais il est interdit de se connecter en local ou physiquement avec un compte à privilège du domaine. Il est important que le mot de passe du compte local ne soit pas réutilisé ailleurs ou prévisible (utiliser LAPS).
 - Un compte d'administration ne doit jamais être utilisé en dehors du cadre d'administration. Concrètement, il ne doit pas être utilisé pour se connecter sur des services où le mot de passe est fourni en direct (ex: sharepoint, OWA, ftp, ssh, authentification web, ...) ; Sauf si vous utilisez le protocole d'authentification IWA [integrated windows authentification]. Les comptes d'administration peuvent consulter des partages de fichier mais seulement depuis le poste d'administration légitime à utiliser le compte, jamais depuis un serveur ou poste administré en indiquant les identifiants du compte d'administration.
 ...
 - Sur un serveur ou un poste à administrer, vous avez le droit de monter vos privilèges en local en utilisant "exécuter en tant que" avec le compte administrateur local du serveur ou du poste mais jamais avec un compte à privilège du domaine. Si vous avez besoin de réaliser des opérations sur le réseau avec des privilèges pour la machine (partage de fichier par exemple), alors vous donnerez les droits au nom du serveur ou nom du poste ou à l'utilisateur simple du poste pour accéder à la ressource réseau.
 - Sur un serveur ou un poste à administrer, vous ne devez jamais créer de service ou de taches planifiées en utilisant un compte à privilège du domaine. Pour créer ce type d'élément, vous devez le créer avec le compte de service local ou domaine avec des droits limités aux besoins et à la machine concernée.
 ...
 - Si vous n'utilisez pas "psexec/admin sharing" pour administrez le parc, alors désactivez le par GPO sur l'ensemble du parc (https://docs.microsoft.com/fr-fr/troubleshoot/windows-server/networking/remove-administrative-shares -- "REG ADD HKLM\SYSTEM\CurrentControlSet\Services\LanmanServer\Parameters /v AutoShareServer /t REG_DWORD /d 0 /f && sc stop "LanmanServer" && sc start "LanmanServer"" ). L'utilisation de psexec n'entraine pas de risque de vol d'identifiant sauf avec un compte où la délégation est activée ou avec l'utilisation de l'option "-u".
 - Si vous n'utilisez pas powershell à distance (WinRM), désactivez le: "powershell Disable-PSRemoting -Force". L'utilisation de Winrm n'entraine pas de risque de vol d'identifiant sauf avec un compte où la délégation est activée.
 - Si vous n'utilisez pas WMI à distance, bloquez l'accès distant avec le firewall local de windows: 'netsh advfirewall firewall set rule group="windows management instrumentation (wmi)" new enable=no'. L'utilisation de WMI à distance n'entraine pas de risque de vol d'identifiant sauf avec un compte où la délégation est activée.
 ...
 - Mettre en place des stratégies d'authentification afin de limiter l'authentification aux comptes à privilèges exclusivement depuis les postes d'administration (défense en profondeur - [Compound Authentication](https://learn.microsoft.com/fr-fr/windows-server/identity/ad-fs/operations/ad-fs-compound-authentication-and-ad-ds-claims)), ce qui permettra en cas du non-respect de certaines mesures ci-dessus et donc la récupération d'un secret par un attaquant (sur une machine administrée) de ne pas pouvoir l'utiliser si celui-ci n'a pas un accès à votre poste d'administration.
